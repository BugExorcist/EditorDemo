...
public class NotificationCenter   {
    ...
    public static NotificationCenter DefaultCenter
    {
        get {
            ...
            // return ObjectInstance.Create(ref defaultCenter); 
...
//private static NotificationCenter defaultCenter = null;
    ...
    { 
        //Debug.LogError("Update " + this.notifications.Count);
    }
    /// 
    /// 注册观察者
    ///    
    ...
    public void AddObserver(Component observer, string name, Component sender) {
        // 对观察者的名字进行检查
        if (name == null || name == "") { Debug.LogError("在AddObserver函数中指定的是空名称!."); return; }
        ...
        //Debug.LogError("this.notifications.Count " + this.notifications.Count);
    }
...
public void PostNotification(Component aSender, string aName) { PostNotification(aSender, aName, null); }
    ...
    public void PostNotification(Notification aNotification) {
        ...
        if (aNotification.name == null || aNotification.name == "") { Debug.LogError("Null name sent to PostNotification."); return; }
        // 该条消息的所有观察者，通过List将他拉出来对其操作
      //  Debug.LogError("this.notifications.Count " + this.notifications.Count);
        List<Component> notifyList = this.notifications[aNotification.name] as List<Component>;
        if (null == notifyList) {
 
            Debug.LogError("在PostNotification的通知列表中未找到: " + aNotification.name); 
            return;
